...
{% macro normalize_priority(col) %}
    case
        -- Nhóm 1: đã là số hợp lệ trong khoảng 1..4 -> giữ nguyên
        when try_cast({{ col }} as integer) between 1 and 4
            then try_cast({{ col }} as integer)
        -- Nhóm 2: nhãn chữ (schema evolution) -> quy về số theo tài liệu API
        ...
        -- Nhóm 3: dữ liệu hỏng ('P1', 'unknown', '0', '5', '-1', '', NULL) -> NULL
        else null
    end
{% endmacro %}
